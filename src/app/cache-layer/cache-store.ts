...
import {generateTodo} from "../services/todo.service";
...
export class AppDB<T>extends Dexie {
  ...
  saveAll<T>(table:string,items: T[])  {
    return this.table(table).clear().then(()=>this.table(table).bulkAdd(items))
  }
   async  save<T extends {id:number}>(table:string,item:T )  {
    console.info("saving fron cache")
